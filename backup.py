...
# def get_all_paths(G, start_node, end_node, cutoff = 10, target_distance = 10, tollerance = 1):
...
#         #     return
#         # if len(path) >= 1:
#         #     # print(G[current_node][path[-1]][0])
#         #     cumulative_distance += G[current_node][path[-1]][0]['length']
#         path.append(current_node)
#         visited.add(current_node)
#         # running_distance += G[current_node][target]['length']
...
#
# # t = time.time()
# # x = list(get_all_paths(G, start_node, end_node, 50))
# # print(time.time() - t)
#
# for path in get_all_paths(G, start_node, end_node, 50):
#     distance = calculate_path_distance(G, path) / 1000
#     fig, ax = ox.plot_graph_route(G, path, route_linewidth=6, node_size=0, bgcolor='k')
#     if distance_km - tollerance <= distance <= distance_km + tollerance:
#         print(f"Route Found\nDistance: {distance}")
